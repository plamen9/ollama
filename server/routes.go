...
func (s *Server) scheduleCompletion(ctx context.Context, name string, requestOpts map[string]any, keepAlive *api.Duration) (*runnerRef, *api.Options, error) {
	if name == "" {
	...
	if !model.Can(CapCompletion) {
		return nil, nil, errors.New("model does not support completion")
	}
...
func (s *Server) GenerateHandler(c *gin.Context) {
	...
	runner, opts, err := s.scheduleCompletion(c.Request.Context(), req.Model, req.Options, req.KeepAlive)
	if err != nil {
...
func (s *Server) ChatHandler(c *gin.Context) {
	...
	prompt, images, err := chatPrompt(c.Request.Context(), runner, req.Messages)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
